            ...
            <ul tabIndex={0} role="menu" className="menu menu-sm dropdown-content bg-base-100 rounded-box z-1 mt-3 w-52 p-2 shadow">
              {menuLinks.map((menuLink) => (
                <li key={menuLink.route} role="none">
                    <Link role="menuitem" className={cn((route() as any).current(menuLink.route) ? 'menu-active' : '')} href={route(menuLink.route)}>
                    {menuLink.name}
                  </Link>
                </li>
              ))}
                      ...
                      <li key={adminLink.route} role="none">
                        <Link
                          ...
                          href={route(adminLink.route)}
                            className={cn((route() as any).current(adminLink.route) ? 'menu-active' : '')}
                        >
                          {adminLink.name}
                        </Link>
                      </li>
              ...
              <li key={menuLink.route} role="none">
                <Link
                  ...
                  href={route(menuLink.route)}
                    className={cn((route() as any).current(menuLink.route) ? 'menu-active' : '')}
                >
                  {menuLink.name}
                </Link>
              </li>
